...
HRESULT My::CCamera::Init()
{
	...
	if (m_pCameraState != nullptr)
	{
		...
		m_pCameraState->TrailView(this);
		m_pCameraState->DiagonalView(this);
 ...
 * @brief デバッグ時のカメラ変更
 */
void My::CCamera::DebugChangeState()
{
	//キーボード情報取得
	CInputKeyboard* pKeyboard = GET_INPUT_KEYBOARD;
	if (pKeyboard->GetTrigger(DIK_F2))
	{
		ChangeCameraState(new CFreeView);
	}
	else if (pKeyboard->GetTrigger(DIK_F3))
	{
		ChangeCameraState(new CThirdView);
	}
	else if (pKeyboard->GetTrigger(DIK_F4))
	{
		ChangeCameraState(new CTrailView);
	}
	else if (pKeyboard->GetTrigger(DIK_F5))
	{
		ChangeCameraState(new CSideView);
	}
}
#endif