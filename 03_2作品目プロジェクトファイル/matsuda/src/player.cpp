...
HRESULT My::CPlayer::Init()
{
	...
	if (m_pBelt == nullptr)
	{
		...
		m_pBelt = CBelt::Create({ parts->GetMtxWorld()._41,parts->GetMtxWorld()._42 - CORRECTION_VALUE_BELT,parts->GetMtxWorld()._43 }, { FLOAT_ZERO,fAngle,FLOAT_ZERO });
		m_pBelt->SetisDraw(GetisDraw());
	}
...
void My::CPlayer::Update()
{
	...
	{//帯があったら
	 //帯を動かす
		CModel_Parts* parts = GetModelParts(0);
		float fAngle = atan2f(sinf(GetRot().y), cosf(GetRot().y));
		m_pBelt->SetPos({ parts->GetMtxWorld()._41 + GetMove().x,parts->GetMtxWorld()._42 - CORRECTION_VALUE_BELT,parts->GetMtxWorld()._43 + GetMove().z});
		m_pBelt->SetRot({ FLOAT_ZERO,fAngle,FLOAT_ZERO });
	}
	Motion(); //モーション処理
