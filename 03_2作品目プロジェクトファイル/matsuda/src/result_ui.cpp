...
//=============================================
#include "result_ui.h"
#include "../../library_kasai/result_rank.h" //のちのちincludeの仕方調整
...
My::CResult_Ui* My::CResult_Ui::Create(D3DXVECTOR3 pos)
{
	...
	//スコアの取得
	int score = 50000;	//Todo:スコアの処理ができたらスコア取得関数を右辺に呼ぶ
	...
	// 初回のみバイナリファイルの書き込みを行う
	static bool isFirstCall = true;
	if (isFirstCall)
	{
		My::CResult_Rank::WriteBin("data\\TXT\\result_rank.bin");
		isFirstCall = false;
	}

	//ランク設定処理
	std::string Path = My::CResult_Rank::SetRank("data\\TXT\\result_rank.bin",score);

	//テクスチャ登録
	pResult_Ui->BindTexture(pTexture->GetAddress(pTexture->Regist(&Path)));
